...
if __name__ == '__main__':
    ...
    variable_sites = {}
    fig, axs = plt.subplots(len(snames),2, figsize=(15,12),
    ...
    for cmin, cmax, a in [(0.03, 1.0, 0.9), (0.01,0.03, 0.6), (0.003,0.01, 0.3)]:
        for i in range(4):
            ...
            ax.scatter(all_pos[ind], (1-major_freqs[sname][ref])[ind], c='C%d'%i,
    ...
    axs[0].legend(loc=1,ncol=2, fontsize=0.8*fs, frameon=True, edgecolor='k')
    axs[-1].set_xlabel('position in genome', fontsize=fs)
    axs[0].set_ylabel('frequency', fontsize=fs)

    for p in primer_boundaries[ref]:
        y = 0.004 if int(p[1])%2 else 0.006
        axs[-1].plot([primer_boundaries[ref][p]['start'], primer_boundaries[ref][p]['end']],[y,y], lw=7, c=(0.7, 0.7, 0.7))
    ...
    for si,sname in enumerate(snames):
        ...
        corr_matrix/=tmp
        axs[si].matshow(corr_matrix, vmin=-1, vmax=1, cmap='RdBu_r')
        axs[si].set_xlabel('SNV 1')
        axs[si].set_ylabel('SNV 2')
    ...
    plt.tight_layout()
    for fmt in ['.pdf', '.png']:
        plt.savefig('figs/dual_infections_Fig5'+fmt)
